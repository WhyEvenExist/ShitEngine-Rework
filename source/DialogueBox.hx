...
class UsefulBox extends FlxSpriteGroup
{
	var box:FlxSprite;

	var dialogue:FlxTypeText;
	var dialogueList:Array<String> = [];
	var dialogueEffects:Array<String> = [];

	public var finishThing:Void->Void;

	var dSounds:Array<FlxSound> = [];

	public function new(talkingRight:Bool = true, ?dialogueList:Array<String>, ?dialogueEffects:Array<String>)
	{
		super();

		box = new FlxSprite(40);
		box.frames = Paths.getSparrowAtlas('dialogue/THINGY');
		box.animation.addByPrefix('normalOpen', 'bad_open', 12, false);
		box.animation.addByPrefix('normal', 'bad_static', 24);
		box.animation.play('normalOpen');
		...
		// dialogue = new Alphabet(0, 80, "", false, true);
		dialogue = new FlxTypeText(0, 80, 32, "", 32);
		dialogue.font = 'assets/fonts/renogare.otf';
		// dialogue.x = 90;
		add(dialogue);
	...
	function startDialogue():Void
	{
		...
		// curCharacter = splitName[1];
		// dialogueList[0] = dialogueList[0].substr(splitName[1].length + 2).trim();
		var theDialog = new FlxTypeText(360, 80, 500, dialogueList[0], 32);
		theDialog.font = 'assets/fonts/renogare.otf';
		dialogue = theDialog;
		// dialogue
		dialogue.sounds = dSounds;
		// dialogue.finishSounds = true;
		add(theDialog);
		dialogue.start(0.09);
	}
}
